...
void DataManager::loadUsers()
{
    ...
    while (std::getline(userFile, line))
    {
        ...
        User user(username, name, phonenum, password, address, balance);
        users_.push_back(user);
    }
    userFile.close();
}

void DataManager::loadCouriers()
{
    std::ifstream courierFile("couriers.txt");
    if (!courierFile.is_open()) return;
...
void DataManager::loadAdmin()
{
    std::ifstream adminFile("admin.txt");
    if (!adminFile.is_open()) return;
...
void DataManager::saveData()
{
    ...
    if (packageFile.is_open())
    {
        ...
        {
            int type = 3; std::string detail = "0";
...
void DataManager::addUser(const User &user) { users_.push_back(user); }
...
void DataManager::addCourier(const Courier &courier) { couriers_.push_back(courier); }
...
int DataManager::getNextCourierId() { return nextCourierId_++; }
...
void DataManager::addPackage(Package *package) { packages_.push_back(package); }
...
int DataManager::getNextPackageId() { return nextPackageId_++; }